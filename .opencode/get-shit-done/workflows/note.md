...
- **Global scope**: `/Users/ybx/ascend-agent/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/ybx/ascend-agent/.opencode/notes/`)
...
2. Glob `/Users/ybx/ascend-agent/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (/Users/ybx/ascend-agent/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/ybx/ascend-agent/.opencode/notes/` — works in any directory
