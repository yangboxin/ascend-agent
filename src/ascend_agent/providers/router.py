...
class ChatResponse:
    ...
    Attributes:
        content: The text content from the assistant, or empty string.
        tool_calls: Native tool calls from the provider (OpenAI format).
            Each dict has keys: id, type, function (with name, arguments).
...
class ModelRouter:
    ...
    def chat(
        ...
        """Send a chat request to the active provider with optional tool calling.
        ...
        Returns:
            ChatResponse with content, tool_calls, and finish_reason.
        """
        params: dict[str, Any] = {
            "model": self._model,
            "messages": messages,
    ...
    def chat_stream(
        ...
        Yields text chunks as they arrive from the API. When a complete
        tool_call is accumulated, yields a ChatResponse with tool_calls
        populated as the final item in the stream.
        ...
        Yields:
            str chunks of the assistant response, with a final ChatResponse
            if tool calls were received.
        """
        params: dict[str, Any] = {
            "model": self._model,
            "messages": messages,
        ...
        if accumulated_tool_calls:
            sorted_calls = [accumulated_tool_calls[i] for i in sorted(accumulated_tool_calls)]
            yield ChatResponse(
                content="",
